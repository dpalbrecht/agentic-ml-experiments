...
df_eng["f0_x_f2"] = df["feature_0"] * df["feature_2"]
df_eng["f1_sq"]   = df["feature_1"] ** 2
all_features = features + ["f0_x_f2", "f1_sq"]
X = df_eng[all_features].values
print(f"Original features: {len(features)}")
print(f"Engineered features added: f0_x_f2 (feature_0 × feature_2), f1_sq (feature_1²)")
print(f"Total features: {X.shape[1]}")
print(f"Target distribution: {dict(zip(*np.unique(y, return_counts=True)))}")

# --- Build model (best settings from experiment 005) ---
print("\n=== Building model ===")
...
print("Best HGBT config from 005 + 2 engineered features (f0_x_f2, f1_sq)")
...
for feat, val in importance.head(10).items():
    marker = " ← engineered" if feat in ("f0_x_f2", "f1_sq") else ""
    print(f"  {feat}: {val:+.4f}{marker}")
print(f"\nEngineered feature ranks: "
      f"f0_x_f2=#{list(importance.index).index('f0_x_f2')+1}, "
      f"f1_sq=#{list(importance.index).index('f1_sq')+1}")
...
print(f"  008-hgbt-eng (this): {this:.4f}  (vs best prev: {this - best_prev:+.4f})")
