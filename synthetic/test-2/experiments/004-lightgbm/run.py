...
def find_best_threshold_pr(y_true, y_proba):
    ...
    # Drop the last precision/recall value (corresponds to recall=0)
    precisions = precisions[:-1]
    recalls = recalls[:-1]
    # Compute F2 at each threshold
    f2_scores = (5 * precisions * recalls) / (4 * precisions + recalls + 1e-10)
    best_idx = np.argmax(f2_scores)
    return thresholds[best_idx], f2_scores[best_idx]


print(f"Running sliding window CV with temporal validation threshold tuning...")
...
for fold_i, test_month in enumerate(tqdm(test_months, desc="Folds")):
    ...
    # --- Threshold tuning via temporal validation split ---
    # Use last month of training window as validation
    val_month = train_months_range[-1]
    sub_train_months = train_months_range[:-1]
    ...
    print(f"  Fold {fold_i+1}: test={test_month}  threshold={best_threshold:.2f}  "
          ...
          f"(val={val_month}, val_F2={val_f2:.4f})")
...
print("\nPer-fold breakdown:")
print(f"{'Fold':>4s}  {'Test':>10s}  {'Val':>10s}  {'Thresh':>6s}  {'F2':>7s}  {'Recall':>7s}  {'Prec':>7s}  {'Val F2':>7s}")
for r in fold_results:
    print(f"{r['fold']:4d}  {r['test_month']:>10s}  {r['val_month']:>10s}  {r['threshold']:6.2f}  "
          f"{r['f2']:7.4f}  {r['recall']:7.4f}  {r['precision']:7.4f}  {r['val_f2']:7.4f}")
